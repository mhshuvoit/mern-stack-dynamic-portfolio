...
class Etc extends React.Component {
    ...
    componentDidMount() {
        Axios.get(ApiEndPoint.baseurl + '/etcs/get')
            .then(result => {
                if (result === null) {
                    this.setState({
                        error: true,
    ...
    contactDelete = () => {
        Axios.delete(ApiEndPoint.baseurl + '/etcs/' + this.state.dataId)
            .then(result => {
                this.setState({
        ...
        if (this.state.spinner === true) {
            ...
            return (
                <Navigation>
                    <Fragment>
                        <Button
                            onClick={this.contactDelete} className='m-3 btn-danger'>{this.state.btnTxt}</Button>
                        <BootstrapTable
                            keyField='_id'
